...
public class TeacherAlterFrm extends JFrame implements ActionListener {		//教师信息修改界面
	...
	ResultSet rs = null;
	String uid;
	public TeacherAlterFrm(String number)
	{
		init();
		setVisible(true);
	...
	public void setText()//此函数功能是 把数据库 需要修改的信息 都打印到界面的文本框中
	{
		...
		try {
			...
			String sql = "select * from teachers where uid = '"+uid+"';";
			...
			else
			{
				...
				if(n == JOptionPane.YES_OPTION)
				{
					...
					try {
						...
						stmt = con.createStatement();
						String str =  "update teachers set uid='"+id+"',upwd='"+pwd+"',name='"+name+"',email='"+email+"',needfindpwd='"+needfindpwd+"' where uid = '"+uid+"';";
